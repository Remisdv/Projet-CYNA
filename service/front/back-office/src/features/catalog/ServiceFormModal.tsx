              ...
              <div className="space-y-6">
                {formData.type === 'product' ? (
                  <div>
                    ...
                    <Input label="Prix () *" type="number" step="0.01" min="0" value={formData.price}
                      onChange={e => updateField('price', parseFloat(e.target.value) || 0)} error={errors.price} placeholder="99.99" />
                  </div>
                ) : (
                  <div className="space-y-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">Tarification Abonnement</h3>
                    <div className="grid grid-cols-2 gap-6">
                      <Input label="Prix mensuel () *" type="number" step="0.01" min="0" value={formData.monthlyPrice}
                        onChange={e => updateField('monthlyPrice', parseFloat(e.target.value) || 0)} error={errors.monthlyPrice} placeholder="49.99" />
                      <Input label="Prix annuel () *" type="number" step="0.01" min="0" value={formData.annualPrice}
                        onChange={e => updateField('annualPrice', parseFloat(e.target.value) || 0)} error={errors.annualPrice} placeholder="509.99" />
                    </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <Input type="number" min="0" value={formData.stockQuantity === 'unlimited' ? '' : formData.stockQuantity}
                        onChange={e => updateField('stockQuantity', parseInt(e.target.value) || 0)} placeholder="Quantité" disabled={formData.stockQuantity === 'unlimited'} />
                    </div>
                  </div>
                </div>
                <Input label="Seuil d'alerte niveau bas" type="number" min="0" value={formData.lowStockThreshold}
                  onChange={e => updateField('lowStockThreshold', parseInt(e.target.value) || 0)} placeholder="10" />
              </div>
            )}
